...
module dual_buffer_bram #(
  ...
  bram_port #(.DATA_WIDTH(BLOCK_RAM_DATA_WIDTH), .ADDR_WIDTH(ADDR_WIDTH)) bram0 (
    .clk(clk), .rst(rst), .wea(wr_en), .waddr(wr_addr), .raddr(rd_addr), .wdata(wr_data_split[0]), .rdata(rd_data_split[0])
  );
  bram_port #(.DATA_WIDTH(BLOCK_RAM_DATA_WIDTH), .ADDR_WIDTH(ADDR_WIDTH)) bram1 (
    .clk(clk), .rst(rst), .wea(wr_en), .waddr(wr_addr), .raddr(rd_addr), .wdata(wr_data_split[1]), .rdata(rd_data_split[1])
  );
  bram_port #(.DATA_WIDTH(BLOCK_RAM_DATA_WIDTH), .ADDR_WIDTH(ADDR_WIDTH)) bram2 (
    .clk(clk), .rst(rst), .wea(wr_en), .waddr(wr_addr), .raddr(rd_addr), .wdata(wr_data_split[2]), .rdata(rd_data_split[2])
  );
  bram_port #(.DATA_WIDTH(BLOCK_RAM_DATA_WIDTH), .ADDR_WIDTH(ADDR_WIDTH)) bram3 (
    .clk(clk), .rst(rst), .wea(wr_en), .waddr(wr_addr), .raddr(rd_addr), .wdata(wr_data_split[3]), .rdata(rd_data_split[3])
  );
