...
bool Perfprocess::get_simulation_stats(long int dse_epoch) {
  // 初始化所有计数器为0
  uint64_t BPWrong = 0;
  uint64_t commitUop = 0;
  uint64_t rob_reads = 0;
  uint64_t rob_writes = 0;
  uint64_t rename_reads = 0;
  uint64_t rename_writes = 0;
  uint64_t fp_rename_reads = 0;
  uint64_t fp_rename_writes = 0;
  uint64_t intdqreads = 0;
  uint64_t intdqwrites = 0;
  uint64_t issue_num = 0;
  uint64_t clock_cycle = 0;
  uint64_t commitInstr = 0;
  uint64_t commitInstrBranch = 0;
  uint64_t jmp_instr_cnt = 0;
  uint64_t div_instr_cnt = 0;
  uint64_t fdiv_fsqrt_instr_cnt = 0;
  uint64_t int_to_float_instr_cnt = 0;
  uint64_t alu_instr_cnt = 0;
  uint64_t store_instr_cnt = 0;
  uint64_t csr_instr_cnt = 0;
  uint64_t load_instr_cnt = 0;
  uint64_t bku_instr_cnt = 0;
  uint64_t fence_instr_cnt = 0;
  uint64_t fmisc_instr_cnt = 0;
  uint64_t fmac_instr_cnt = 0;
  uint64_t fmac_instr_cnt_fma = 0;
  uint64_t mul_instr_cnt = 0;
  uint64_t mou_instr_cnt = 0;
  uint64_t intRegfileReads = 0;
  uint64_t intRegfileWrites = 0;
  uint64_t fpRegfileReads = 0;
  uint64_t fpRegfileWrites = 0;
  uint64_t intAluAccess = 0;
  uint64_t fpuAccess = 0;
  uint64_t mulAccess = 0;
  uint64_t access_itlb = 0;
  uint64_t miss_itlb = 0;
  uint64_t access_ldtlb = 0;
  uint64_t miss_ldtlb = 0;
  uint64_t access_sttlb = 0;
  uint64_t miss_sttlb = 0;
  uint64_t replace_itlb = 0;
  uint64_t replace_ldtlb = 0;
  uint64_t replace_sttlb = 0;
  uint64_t icache_read_access = 0;
  uint64_t icache_read_miss = 0;
  uint64_t icache_conflit = 0;
  uint64_t dcache_read_access = 0;
  uint64_t dcache_read_miss = 0;
  uint64_t dcache_write_access = 0;
  uint64_t dcache_write_miss = 0;
  uint64_t dcache_conflit = 0;
  uint64_t btb_read_access = 0;
  uint64_t btb_write_access = 0;
  uint64_t l2_read_access = 0;
  uint64_t l2_read_miss = 0;
  uint64_t l2_write_access = 0;
  uint64_t l2_write_miss = 0;
  uint64_t l2_conflit = 0;
  uint64_t memory_access = 0;
  uint64_t memory_write = 0;
  
  // 读取性能计数器
  // BPWrong = find_perfCnt("BPWrong");
  // commitUop = find_perfCnt("commitUop");
  // rob_reads = find_perfCnt("rob_reads");
  // rob_writes = find_perfCnt("rob_writes");
  // rename_reads = find_perfCnt("rename_reads");
  // rename_writes = find_perfCnt("rename_writes");
  // fp_rename_reads = find_perfCnt("fp_rename_reads");
  // fp_rename_writes = find_perfCnt("fp_rename_writes");
  // intdqreads = find_perfCnt("intdqreads");
  // intdqwrites = find_perfCnt("intdqwrites");
  // issue_num = find_perfCnt("issue_num");
  clock_cycle = find_perfCnt("clock_cycle");
  // commitInstr = find_perfCnt("commitInstr");
  // commitInstrBranch = find_perfCnt("commitInstrBranch");
  // jmp_instr_cnt = find_perfCnt("jmp_instr_cnt");
  // div_instr_cnt = find_perfCnt("div_instr_cnt");
  // fdiv_fsqrt_instr_cnt = find_perfCnt("fdiv/fsqrt_instr_cnt");
  // int_to_float_instr_cnt = find_perfCnt("int_to_float_instr_cnt");
  // alu_instr_cnt = find_perfCnt("alu_instr_cnt");
  // store_instr_cnt = find_perfCnt("store_instr_cnt");
  // csr_instr_cnt = find_perfCnt("csr_instr_cnt");
  // load_instr_cnt = find_perfCnt("load_instr_cnt");
  // bku_instr_cnt = find_perfCnt("bku_instr_cnt");
  // fence_instr_cnt = find_perfCnt("fence_instr_cnt");
  // fmisc_instr_cnt = find_perfCnt("fmisc_instr_cnt");
  // fmac_instr_cnt = find_perfCnt("fmac_instr_cnt");
  // fmac_instr_cnt_fma = find_perfCnt("fmac_instr_cnt_fma");
  // mul_instr_cnt = find_perfCnt("mul_instr_cnt");
  // mou_instr_cnt = find_perfCnt("mou_instr_cnt");
  // intRegfileReads = find_perfCnt("intRegfileReads");
  // intRegfileWrites = find_perfCnt("intRegfileWrites");
  // fpRegfileReads = find_perfCnt("fpRegfileReads");
  // fpRegfileWrites = find_perfCnt("fpRegfileWrites");
  // intAluAccess = find_perfCnt("intAluAccess");
  // fpuAccess = find_perfCnt("fpuAccess");
  // mulAccess = find_perfCnt("mulAccess");
  // access_itlb = find_perfCnt("access_itlb");
  // miss_itlb = find_perfCnt("miss_itlb");
  // access_ldtlb = find_perfCnt("access_ldtlb");
  // miss_ldtlb = find_perfCnt("miss_ldtlb");
  // access_sttlb = find_perfCnt("access_sttlb");
  // miss_sttlb = find_perfCnt("miss_sttlb");
  // replace_itlb = find_perfCnt("replace_itlb");
  // replace_ldtlb = find_perfCnt("replace_ldtlb");
  // replace_sttlb = find_perfCnt("replace_sttlb");
  // icache_read_access = find_perfCnt("ICache_read_access");
  // icache_read_miss = find_perfCnt("ICache_read_miss");
  // icache_conflit = find_perfCnt("ICache_conflit");
  // dcache_read_access = find_perfCnt("dcache_read_access");
  // dcache_read_miss = find_perfCnt("dcache_read_miss");
  // dcache_write_access = find_perfCnt("dcache_write_access");
  // dcache_write_miss = find_perfCnt("dcache_write_miss");
  // dcache_conflit = find_perfCnt("dcache_conflit");
  // btb_read_access = find_perfCnt("btb_read_access");
  // btb_write_access = find_perfCnt("btb_write_access");
  // l2_read_access = find_perfCnt("l2_read_access");
  // l2_read_miss = find_perfCnt("l2_read_miss");
  // l2_write_access = find_perfCnt("l2_write_access");
  // l2_write_miss = find_perfCnt("l2_write_miss");
  // l2_conflit = find_perfCnt("l2_conflit");
  // memory_access = find_perfCnt("memory_access");
  // memory_write = find_perfCnt("memory_write");
  ...
  uint64_t total_instructions = int_instructions + fp_instructions + 
                                load_instr_cnt + store_instr_cnt + 
                                csr_instr_cnt + bku_instr_cnt + 
                                fence_instr_cnt + mou_instr_cnt + 
                                int_to_float_instr_cnt;

  // 输出到文件
  std::ostringstream stats;
  stats << "dse_epoch: " << dse_epoch << std::endl
        << "total_cycles: " << clock_cycle << std::endl
        << "total_instructions: " << total_instructions << std::endl
        << "issue_num: " << issue_num << std::endl
        ...
        << "jmp_instr_cnt: " << jmp_instr_cnt << std::endl
        << "div_instr_cnt: " << div_instr_cnt << std::endl
        << "fdiv_fsqrt_instr_cnt: " << fdiv_fsqrt_instr_cnt << std::endl
        << "int_to_float_instr_cnt: " << int_to_float_instr_cnt << std::endl
        << "alu_instr_cnt: " << alu_instr_cnt << std::endl
        << "csr_instr_cnt: " << csr_instr_cnt << std::endl
        << "bku_instr_cnt: " << bku_instr_cnt << std::endl
        << "fence_instr_cnt: " << fence_instr_cnt << std::endl
        << "fmisc_instr_cnt: " << fmisc_instr_cnt << std::endl
        << "fmac_instr_cnt: " << fmac_instr_cnt << std::endl
        << "fmac_instr_cnt_fma: " << fmac_instr_cnt_fma << std::endl
        << "mul_instr_cnt: " << mul_instr_cnt << std::endl
        << "mou_instr_cnt: " << mou_instr_cnt << std::endl
        << "commitInstr: " << commitInstr << std::endl;
