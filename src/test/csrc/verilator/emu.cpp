    ...
    switch (o) {
      case 0:
        switch (long_index) {
          ...
          case 29:
#ifdef TRACERTL_MODE
            args.enable_gen_paddr = true; continue;
#else
            printf("[WARN] tracertl is not enabled at compile time, ignore --gen-paddr\n");
            continue;
#endif // TRACERTL_MODE
          case 30:
#ifdef ENABLE_CHISEL_MAP
...
int Emulator::tick() {
  ...
  do {
    auto trap = difftest[0]->get_trap_event();
    static bool warmup_inst_finished = false;
    if (!warmup_inst_finished && trap->instrCnt >= args.warmup_instr) {
      warmup_inst_finished = true;
