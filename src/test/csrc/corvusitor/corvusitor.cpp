...
CorvusitorSim::CorvusitorSim() : dut(new VCorvusTopWrapper) {}

CorvusitorSim::~CorvusitorSim() {
#if VM_TRACE == 1
  if (waveform) {
    delete waveform;
    waveform = nullptr;
...
#if VM_TRACE == 1
void CorvusitorSim::waveform_init(uint64_t cycles) {
  waveform = new EmuWaveform(trace_bind, cycles);
}

void CorvusitorSim::waveform_init(uint64_t cycles, const char *filename) {
  waveform = new EmuWaveform(trace_bind, cycles, filename);
}

void CorvusitorSim::waveform_tick() {
  waveform->tick();
}
#endif // VM_TRACE == 1

#ifdef VM_SAVABLE
void CorvusitorSim::snapshot_init() {
  snapshot_slot = new VerilatedSaveMem[2];
}

void CorvusitorSim::snapshot_save(int index) {
  if (snapshot_slot) {
    if (index >= 0) {
...
std::function<void(void *, size_t)> CorvusitorSim::snapshot_take() {
...
std::function<void(void *, size_t)> CorvusitorSim::snapshot_load(const char *filename) {
