...
public:
  ...
  virtual uint32_t get_covered_points() { return get_acc_covered_points(); }
  ...
  virtual bool is_accumulated(uint32_t i) { return false; }
...
public:                                                           \
  const char* get_name() { return cov_name; }                     \
  ...
  uint32_t get_covered_points() { return sum(info); }             \
                                                                  \
  void accumulate() {                                             \
  ...
  bool is_accumulated(uint32_t i) { return acc[i].covered; }      \
  uint32_t get_acc_covered_points() { return sum(acc); }          \
                                                                  \
...
BASIC_COVER_FROM_DIFF(
  "Instruction",
  Instr,
  DifftestInstrCover,
  icover,
  CONFIG_DIFF_ICOVER_WIDTH
)
...
BASIC_COVER_FROM_DIFF(
  "Instr-Imm",
  InstrImm,
  DifftestInstrImmCover,
  instr_imm_cover,
  CONFIG_DIFF_INSTR_IMM_COVER_WIDTH
)
...
public:
  FIRRTLCoverage();
  ~FIRRTLCoverage();
  const char* get_name() { return "FIRRTL"; };
  void reset();
...
public:
  LLVMSanCoverage() {};
  inline const char* get_name() { return "llvm.branch"; }
  inline const char *get_cover_name(uint32_t i) { return llvm_sancov->info[i].c_str(); }
  inline bool is_accumulated(uint32_t i) { return llvm_sancov->points[i]; }
  inline void reset() {}

  // coverage figures
  inline uint32_t get_total_points() { return llvm_sancov->points.size(); }
  inline void accumulate() {}

  // accumulative coverage
  inline uint32_t get_acc_covered_points() { return llvm_sancov->reach; }
...
public:
  ...
  const char* get_name() { return "union"; }
  void reset();
