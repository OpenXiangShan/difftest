...
template <typename... Args> std::string sfmt(const std::string &format, Args... args) {
...
static inline int128_t swap(int128_t n) {
    ...
    for (unsigned i = 0; i < bswap(eh->e_phnum); i++) {                                                      \
      if (bswap(ph[i].p_type) == PT_LOAD && bswap(ph[i].p_memsz)) {                                          \
        ...
        if (size_t pad = bswap(ph[i].p_memsz) - bswap(ph[i].p_filesz)) {                                     \
          zeros.resize(pad);                                                                                 \
          memif->write(bswap(ph[i].p_paddr) + bswap(ph[i].p_filesz), pad, zeros.data());                     \
        }                                                                                                    \
      }                                                                                                      \
    }                                                                                                        \
  ...
  if (IS_ELFLE(*eh64)) {
    if (memif->get_target_endianness() != endianness_little) {
      throw std::invalid_argument(
          "Specified ELF is little endian, but system uses a big-endian memory system. Rerun without --big-endian");
    }
    if (IS_ELF32(*eh64))
    ...
    if (memif->get_target_endianness() != endianness_big) {
      throw std::invalid_argument(
          "Specified ELF is big endian, but system uses a little-endian memory system. Rerun with --big-endian");
    }
    if (IS_ELF32(*eh64))
...
bool SparseRam::add_blk(char *name, paddr_t start, paddr_t end) {
  ...
  auto blk = new sp_mm_blk{.start = start, .end = end, .blk = (u_int8_t *)calloc(end - start, sizeof(u_int8_t))};
...
void SparseRam::copy(SparseRam *dst) {
  ...
  auto fc = [&](paddr_t addr, size_t len, void *buff) { dst->write(addr, len, buff); };
  this->copy_nzero_bytes(fc);
}

void SparseRam::print_info() {
  OUTPUT(stderr, "SpRam blocks: %ld, size: %.2f MB\n", this->mem.size(),
         float(this->mem.size() * this->block_size) / (1024.0 * 1024.0));
}
