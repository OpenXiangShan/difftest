...
public:
  QueryBase(sqlite3 *db) {
    query_db = db;
  }
  ~QueryBase() {
    sqlite3_finalize(pPrepare);
  }
  virtual const char *get_createSql() = 0;
  virtual const char *get_insertSql() = 0;
  // Call in subClass constructor function
  virtual void initTable() {
    char *errMsg;
    int rc;
    rc = sqlite3_exec(query_db, get_createSql(), 0, 0, &errMsg);
    if (rc != SQLITE_OK) {
      printf("SQL error: %s\n", errMsg);
      assert(0);
    }
    const char *insertSql = get_insertSql();
