...
class DifftestInterfaces extends Module {
  ...
  val arch_int_reg_state = DifftestModule(new DiffArchIntRegState, dontCare = true)
  val arch_fp_reg_state = DifftestModule(new DiffArchFpRegState, dontCare = true)
  val arch_vec_state = DifftestModule(new DiffArchVecRegState, dontCare = true)
  ...
  // These bundles will be replaced by architectural register state in Preprocess.scala.
  // val arch_int_rename_table = DifftestModule(new DiffArchIntRenameTable(192), dontCare = true)
  // val arch_fp_rename_table = DifftestModule(new DiffArchFpRenameTable(128), dontCare = true)
  // val arch_vec_rename_table = DifftestModule(new DiffArchVecRenameTable(64), dontCare = true)
  // val phy_int_reg_state = DifftestModule(new DiffPhyIntRegState(192), dontCare = true)
  // val phy_fp_reg_state = DifftestModule(new DiffPhyFpRegState(128), dontCare = true)
  // val phy_vec_reg_state = DifftestModule(new DiffPhyVecRegState(64), dontCare = true)
