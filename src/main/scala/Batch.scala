...
//
  ...
  def TruncDataBitLen = math.min(MaxDataBitLen, StepDataBitLen)
  def TruncInfoBitLen = math.min(MaxInfoBitLen, StepInfoBitLen)
}
...
//  val group_grains_hint = ListBuffer.empty[Seq[Int]]
  sorted.zipWithIndex.foreach{ case (v_gens, idx) =>
    ...
    group_data(idx) := VecInit(aligned.zip(valids).zipWithIndex.map{ case ((gen, v), i) =>
      val shifted = if (i != 0) {
  ...
  (1 until param.StepGroupSize).foreach{idx =>
    val cat_map = Seq.tabulate(group_length(idx) + 1){ len =>
      (len.U, Cat(step_data_head(idx - 1),
        delay_group_data(idx)(len * group_bitlen(idx) - 1, 0)))
    }
...
//        }
        val append_step_data = (delay_step_data(param.TruncDataBitLen - 1, 0) << (state_status.data_bytes << 3).asUInt).asUInt
        val append_step_info = (delay_step_info(param.TruncInfoBitLen - 1, 0) << (state_status.info_size * param.infoWidth.U)).asUInt
