...
object Delta {
  ...
  def collect(): Unit = {
    ...
    val deltaInsts = instances.filter(_.doDelta)
...
class DeltaEndpoint(bundles: Seq[Valid[DifftestBundle]], config: GatewayConfig) extends Module {
  ...
  val deltas = in.filter(_.bits.doDelta).flatMap { v_gen =>
  ...
  val withDeltas = MixedVecInit((in.filterNot(_.bits.doDelta) ++ deltas).toSeq)
