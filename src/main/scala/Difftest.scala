...
object DifftestModule {
  ...
  def top[T <: RawModule with HasDiffTestInterfaces](
    cpuGen: => T,
    modPrefix: String,
    createTopIO: Boolean,
  ): SimTop[T] = {
    new SimTop(cpuGen, modPrefix, createTopIO)
  }

  def top[T <: RawModule with HasDiffTestInterfaces](cpuGen: => T): SimTop[T] = top[T](cpuGen, "", true)
