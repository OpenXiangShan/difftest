  ...
  def collect() = {
    ...
    queryCpp +=
      s"""
         ...
         |
         |${tables.map(_.classDecl).mkString("\n")}
         |class QueryStats: public QueryStatsBase {
         |public:
         |  ${tables.map(_.classInit).mkString("\n  ")}
         |  QueryStats(char *path): QueryStatsBase(path) {
         |    ${tables.map(_.classAssign).mkString("\n    ")}
         |  }
...
class QueryTable(val gen: DifftestBundle, locPrefix: String) {
  ...
  val classType: String = tableName + "Query"
  val className: String = "query_" + tableName
  val classDecl: String =
    s"""
       |class ${classType}: public QueryBase {
       |public:
       |  ${classType}(sqlite3* db): QueryBase(db) {
       |    initTable();
       |  }
       |  const char* get_createSql() {
       |    return \" CREATE TABLE $tableName(\" \\
       |      "ID INTEGER PRIMARY KEY AUTOINCREMENT," \\
       |      ${sqlArgs.map("\"" + _._1 + " INT NOT NULL").mkString("", ",\" \\\n      ", ");\";")}
       |  }
       |  const char* get_insertSql() {
       |    return \"INSERT INTO $tableName (${sqlArgs.map(_._1).mkString(",")}) \" \\
       |      \" VALUES (${Seq.fill(sqlArgs.length) { "?" }.mkString(",")});\";
       |  }
       |};
       |"""
  val classInit = s"${classType}* ${className};"
  val classAssign = s"${className} = new ${classType}(mem_db);"
  val writeDecl =
    s"""
