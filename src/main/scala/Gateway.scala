...
{
  def dutBufSize: Int = if (isBatch) batchSize else 1
  def needEndpoint: Boolean = hasGlobalEnable || isBatch
}
...
class GatewayEndpoint(signals: Seq[DifftestBundle], config: GatewayConfig) extends Module {
  ...
  val stepWidth = if (config.isBatch) log2Ceil(config.batchSize+1) else 1
  val upper = if(config.isBatch) config.batchSize.U else 1.U
  val step = IO(Output(UInt(stepWidth.W)))
