...
object Preprocess {
  ...
  def getVecCommitData(
    ...
    if (bundles.exists(_.desiredCppName == "wb_vec")) {
      ...
      commits.map { c =>
        val otherData = c.otherwpdest.zip(c.otherV0Wen).map { case (pdest, v0Wen) =>
          WireInit(Mux(v0Wen, v0PhyRf(c.coreid)(pdest), vecPhyRf(c.coreid)(pdest)))
        }
        ...
        when(c.valid) {
          c.otherwpdest.zipWithIndex.map { case (pdest, i) =>
            when(c.otherV0Wen(i)) {
              for (v0Wb <- v0WriteBacks) {
                when(v0Wb.valid && v0Wb.coreid === c.coreid && v0Wb.address === pdest) {
                  otherData(i) := v0Wb.data
                }
              }
            }.otherwise {
              for (vecWb <- vecWriteBacks) {
...
class PreprocessEndpoint(bundles: Seq[DifftestBundle]) extends Module {
  ...
  val commitData = commits.zip(fpData).zip(vecData).zip(intData).map { case (((c, f), v), i) =>
    ...
    val vectoreWen = c.v0wen || c.vecwen
    cd.coreid := c.coreid
    cd.index := c.index
    cd.valid := c.valid
