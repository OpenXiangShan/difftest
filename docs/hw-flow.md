...
- Optional `+iotrace-pace-cycle`: gate DPI `enable` until the next `TrapEvent.cycleCnt` is due and emit zeros for the skipped DUT cycles. Default is packed (one record per gated cycle).
